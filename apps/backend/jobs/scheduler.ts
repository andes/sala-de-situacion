...
class Scheduler {
    ...
    static initialize() {
        ...
        log('initialize');
        environment.jobs.forEach((job: any) => {
            this.runningProccess[job.action] = [];
            schedule.scheduleJob(job.when, () => {
                const concurrent = !!job.concurrent;
                if (concurrent || this.runningProccess[job.action].length === 0) {
                    ...
                    child.timer = this.preventDeadLock(job, child);
                    this.runningProccess[job.action].push(child);
